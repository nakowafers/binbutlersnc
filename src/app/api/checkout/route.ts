    ...
    try {
        ...
        if (mode === 'payment' && validatedData.setup_fee_override !== undefined) {
            lineItems.push({
                price_data: {
                    currency: 'usd',
                    product: env.STRIPE_ONETIME_PRICE_ID.startsWith('price_')
                        ? (await stripe.prices.retrieve(env.STRIPE_ONETIME_PRICE_ID)).product as string
                        : env.STRIPE_ONETIME_PRICE_ID,
                    unit_amount: validatedData.setup_fee_override * 100,
                },
                quantity: 1,
            });
        } else {
        ...
        if (mode === 'subscription') {
            if (validatedData.setup_fee_override !== undefined) {
                ...
                // We need the product ID from the setup fee price
                const price = await stripe.prices.retrieve(env.STRIPE_SETUP_FEE_PRICE_ID);
                lineItems.push({
                    price_data: {
                        currency: 'usd',
                ...
                lineItems.push({
                    price: env.STRIPE_SETUP_FEE_PRICE_ID,
                    quantity: 1,
                });
            }
        }
